...
EbN0 = -2:1:16;                                 % Eb to N0 ratio  (Eb = bit energy, N0 = noise PSD)  -> vector to compare BER
...
for i =1:length(EbN0)
    ...
    cropped_filtered_signal_rx_sync_errors(i,:) = filtered_signal_rx_sync_errors(i,N:end-(N-1)).*exp(-1j*(2*pi*CFO*t+phase_offset));
end
